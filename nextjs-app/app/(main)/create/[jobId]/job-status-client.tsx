/**
 * @file job-status-client.tsx
 * @description
 *  Client component for interactive portions of the job status page.
 *  Handles approval dialog, deck selection, and fallback polling.
 *  Receives initial server-rendered data as props.
 *
 * @dependencies
 *  - Next.js (useRouter)
 ...
 *  - Server actions (reviewCardsAction, getDecksAction)
 */
...
/**
 * Client component that handles interactive portions and fallback polling.
 */
export function JobStatusClient({ initialData, jobId }: JobStatusClientProps) {
  const router = useRouter();
  
  const [jobState, setJobState] = useState<JobData>(initialData);
  ...
  // Fallback polling for job status (only if status is not terminal)
  useEffect(() => {
    ...
    if (jobState.status !== "completed" && jobState.status !== "failed") {
      async function fetchJobStatus() {
        try {
          const res = await fetch(`/api/job-status/${jobId}`, {
            method: "GET",
            cache: "no-store", // Bypass cache for polling
          });
          if (!res.ok) {
          ...
          // Clear interval if job is now complete
      ...
      // Start polling with 30-second interval (increased from 3 seconds)
      intervalId = setInterval(fetchJobStatus, 30000);
    }
